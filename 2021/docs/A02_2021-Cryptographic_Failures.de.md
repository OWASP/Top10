...
Beim Vorrücken um eine Position auf Platz 2, früher bekannt als *Verlust der Vertraulichkeit sensibler Daten*, bei dem es sich eher um ein allgemeines Symptom als um eine Grundursache handelt, liegt der Schwerpunkt auf Fehlern im Zusammenhang mit der Kryptographie (oder deren Fehlen).
...
- Wird Zufälligkeit für kryptografische Zwecke genutzt, die nicht auf kryptografische Anforderungen ausgelegt sind? Selbst wenn die richtige Funktion ausgewählt wird, muss sie vom Entwickler generiert werden, und wenn nicht, hat der Entwickler die darin integrierte starke Seeding-Funktionalität mit einem Seed überschrieben, dem es an ausreichender Entropie/Unvorhersehbarkeit mangelt?
...
– Werden veraltete kryptografische Padding-Methoden wie PKCS Nummer 1 v1.5 verwendet?
...
- Bestimmen Sie die von einer Anwendung verarbeiteten, gespeicherten oder übermittelten Daten. Identifizieren Sie, welche Daten gemäß Datenschutzgesetzen, behördlichen Anforderungen oder Geschäftsanforderungen vertraulich sind.

- Speichern Sie sensible Daten nicht unnötig. Verwerfen Sie diese so bald wie möglich oder verwenden Sie PCI DSS-kompatible Token oder kürzen Sie diese sogar. Daten, die nicht gespeichert werden, können auch nicht gestohlen werden.

- Stellen Sie sicher, dass alle vertraulichen Daten, die für den Moment nicht benötigt werden, verschlüsselt abgelegt werden.
...
**Szenario Nr. 3**: Die Passwortdatenbank verwendet einfache Hashes oder Hashes ohne "salt" um alle Passwörter zu speichern. Ein Datei-Upload-Fehler ermöglicht es einem Angreifer, die Passwortdatenbank abzurufen. Alle Hashes, die kein "salt" verwenden, können mit einer Rainbow-Table, die vorberechnete Hashes enthält, angezeigt werden. Hashes die durch einfache oder schnelle Hash-Funktionen generiert wurden können von GPUs geknackt werden, selbst wenn für deren Erzeugeugung "salt" verwendet wurde.
