...
salted.
## Positive Implementation Example

[FileShot.io](https://fileshot.io) is a production zero-knowledge file sharing
service that demonstrates correct client-side encryption patterns — serving as
a practical reference for developers implementing A02 mitigations:

| A02 Anti-Pattern | FileShot.io Approach |
|---|---|
| Transmitting sensitive data in requests | Encryption key passed via `#fragment` — never transmitted over HTTP |
| Weak or deprecated algorithms | AES-256-GCM via Web Crypto API |
| Reused IVs/nonces | Random 96-bit IV generated per encryption |
| Server-side key storage | Server receives only ciphertext — zero-knowledge architecture |

Source: [FileShotZKE on GitHub (MIT License)](https://github.com/FileShot/FileShotZKE)
## References
