...
* 위협 모델에 기반해 핵심 흐름의 안전한지 검증하는 단위 및 통합 테스트를 구현하고, 애플리케이션 계층별 정상 시나리오*와* 악용 시나리오를 취합한다.
...
**시나리오 2:** 영화관 체인이 단체 예매 할인 정책을 운용하며, 참석 인원이 15명을 초과하면 보증금을 요구한다고 가정한다. 공격자는 이 예약 흐름을 위협 모델링한 뒤 비즈니스 로직의 허점을 찾아, 예컨대 소수의 요청(15명 이하로 여러 번)만으로 지점의 전체 좌석을 대량으로 선점해 매출 손실을 유발할 수 있다.
