...
Authentication Failures maintains its position at #7 with a slight name change to more accurately reflect the 36 CWEs in this category. Despite benefits from standardized frameworks appears, this category has kept its #7 rank from 2021. Notable CWEs included are CWE-259 Use of Hard-coded Password, *CWE-297: Improper Validation of Certificate with Host Mismatch*, *CWE-287: Improper Authentication*, *CWE-384: Session Fixation, and CWE-798 Use of Hard-coded Credentials*.
...
When an attacker is able to trick a system into recognizing an invalid or incorrect user as legitimate, this vulnerability is present. There may be authentication weaknesses if the application:
        ...
        ·   	Permits automated attacks such as credential stuffing, where the attacker has a breached list of valid usernames and passwords. More recently this type of attack has been expanded to include hybrid password attacks *rainbow* credential stuffing (also known as password spray attacks), where the attacker uses variations or increments of spilled credentials to gain access, for instance trying Password1!, Password2!, Password3! and so on.
        ...
        ·   	Exposes session identifier in the URL, a hidden field, or another insecure location.
...
## How to prevent. 
        ...
        ·   	During new account creation and password changes validate against lists of known breached credentials (there are services for this).


        ·   	Align password length, complexity, and rotation policies with National Institute of Standards and Technology (NIST) 800-63b's guidelines in section 5.1.1 for Memorized Secrets or other modern, evidence-based password policies.


        ·   	Do not force human beings to rotate passwords unless you suspect breach. If you suspect breach, force password resets immediately. Ideally you would register your users for a service such as [haveibeenowned.com](haveibeenowned.com) or similar to monitor if your users have been in a breach.
...
**Scenario #3:** Application session timeouts aren't implemented correctly. A user uses a public computer to access an application and instead of selecting "logout," the user simply closes the browser tab and walks away. Another Example for this is, if a Single Sign on (SSO) session can not be closed by a Single Logout (SLO). That is, a single login logs you into, for example, your mail reader, your document system, and your chat system. But logging out happens only to the current system. out An attacker uses the same browser after the victim things they have successfully logged outan hour later, butwith the user is stilluser stilluser is still to some of the applications authenticated. The same issue can happen in offices and enterprises when a sensitive application has not been properly exited and a colleague has (temporarily) access to the unlocked computer.
