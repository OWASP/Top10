...
すでにプログラムを運用している場合は、[OWASP SAMM (Software Assurance Maturity Model)](https://owasp.org/www-project-samm/) や DSOMM (DevSecOps Maturity Model) を用いた成熟度評価を検討してください。これらのモデルは包括的であり、プログラムを拡張・成熟させるためにどこに注力すべきかを判断するのに役立ちます。なお、SAMM や DSOMM のすべてを完璧にこなす必要はありません。これらはあくまでガイドであり、組織の状況に応じた多くの選択肢を提示するものです。
...
* セキュリティチャンピオン・プログラムや、開発者向けの普及啓発 (Advocacy) プログラムの開始を検討してください。これにより開発者の知識を最新の状態に保ち、安全に業務を行う方法を伝え、組織のセキュリティ文化をよりポジティブなものにできます。OWASP は [OWASP Security Champions Guide](https://securitychampions.owasp.org/) を通じてこの活動を支援しています。
...
* [OWASP Secure Software Contract Annex](https://owasp.org/www-community/OWASP_Secure_Software_Contract_Annex) などのテンプレートやチェックリストを採用してください。
...
* セキュリティ制御の自作は困難です。可能な限り安全なデフォルトを提供し、最も簡単な方法が最も安全な方法となる「舗装された道 (Paved Roads)」を整備してください。
...
* セキュリティテストツール（ファザー、DAST など）と安全なテスト環境、およびトレーニングを提供します。
* 高いレベルのアシュアランスが必要な場合は、正式なペネトレーションテスト、ストレスデータテスト、パフォーマンステストを検討してください。
* 開発者と協力してバグレポートから修正の優先順位を決定し、修正のための時間を確保できるようにします。
...
* 少なくとも年に一度はペネトレーションテストを実施してください。
...
標準として使用するのが難しい理由の一つに、本ドキュメントが単なるテスト項目ではなく「リスク」を記述している点が挙げられます。例えば「安全でない設計 (Insecure Design)」は、ほとんどのテスト手法の範囲を超えています。また、ロギングと監視が適切に実施されているかの確認は、インタビューやインシデント対応のサンプリング調査を通じてのみ可能です。
...
ツールベンダーにとって、ASVS は唯一の受け入れ可能な選択肢です。ツールだけで OWASP Top 10 を完全に網羅することは不可能であり、OWASP はそのような主張を推奨していません。
